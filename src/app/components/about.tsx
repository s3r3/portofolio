  ...
  const calculateTextDimensions = (width: number, height: number) => {
    ...
    const BASE_CHAR_HEIGHT = 100;
    const BASE_LINE_SPACING = 100;
    const BASE_X_OFFSET = 200;
    const BASE_CHAR_SPACING = -60;
    
    let FONT_SIZE = BASE_FONT_SIZE * scale;
    let CHAR_HEIGHT = BASE_CHAR_HEIGHT * scale;
    let LINE_SPACING = BASE_LINE_SPACING * scale;
    let CHAR_SPACING = BASE_CHAR_SPACING * scale;
    
    // Account for 40px padding-bottom (pb-10)
    const adjustedHeight = height - 40;
    const totalTextHeight = LINE_SPACING * lines.length;
    let verticalScale = totalTextHeight > adjustedHeight ? adjustedHeight / totalTextHeight : 1;
    
    // Check max width to prevent overflow (accounting for 80px padding from p-10)
    const maxWidth = width - 80;
    const measureCtx = document.createElement('canvas').getContext('2d');
    if (measureCtx) {
      measureCtx.font = `900 ${FONT_SIZE}px ${FONT_FAMILY}`;
      const longestLine = lines.reduce((max, line) => Math.max(max, measureCtx.measureText(line.text).width), 0);
      if (longestLine > maxWidth) {
        verticalScale *= maxWidth / longestLine;
      }
    }
...
const GoodreadsWidget = () => {
  useEffect(() => {
    // optional: bisa dipakai kalau mau trigger ulang animasi setelah Goodreads inject konten
  }, []);

  return (
    <div className="my-10">
      <div id="gr_grid_widget_1757519022">
        <h2 className="text-white mb-4 text-xl font-semibold">
          <a
            className="hover:underline"
            rel="nofollow"
            href="https://www.goodreads.com/review/list/179640700-muhammad-farid?shelf=read&utm_medium=api&utm_source=grid_widget"
          >
            Muhammad&apos;s Books
          </a>
        </h2>

        {/* Container buku dengan animasi */}
        <motion.div
          className="gr_grid_container"
      ...
      {/* CSS override untuk Goodreads */}
      <style jsx global>{`
        .gr_grid_container {
          display: flex !important;
          flex-wrap: nowrap !important; /* 1 row saja */
          gap: 16px !important;
        ...
        .gr_grid_container::-webkit-scrollbar {
          height: 8px;
        }
        .gr_grid_container::-webkit-scrollbar-track {
          background: #1f2937;
          border-radius: 4px;
        }
        .gr_grid_container::-webkit-scrollbar-thumb {
          background: #4b5563;
          border-radius: 4px;
        }
        .gr_grid_container::-webkit-scrollbar-thumb:hover {
          background: #9ca3af;
        }

        .gr_grid_book_container {
          float: none !important;
          flex: 0 0 auto !important; /* horizontal */
      ...
      {/* Goodreads script */}
      <Script
        src="https://www.goodreads.com/review/grid_widget/179640700?cover_size=medium&hide_link=&hide_title=&num_books=20&order=&shelf=&sort=&widget_id=1757519022"
        strategy="lazyOnload"
      />
    </div>
  );
  ...
  const About = () => {
    ...
    return (
      <section className="about mt-[200px] h-[300vh] flex flex-col "  id="about">
        <div className='flex w-full justify-center items-center'>
          <h2 className="text-[10rem] font-bold">ABOUT ME</h2>
        </div>
        

        <div className=" bg-[#101010] text-[#aaa8a8] p-10 sticky top-0 left-0 h-screen" >
          <div className="container ">
            {/* Physics text section */}
            <div className="about-first__wrapper absolute w-full left-0 top-0 bg-[#101010]" style={{ zIndex: 20 }}>
              <div className="about-first-top flex justify-between items-center relative z-[11] pt-[100px] px-10">
                <p className="overflow-hidden">
                  <span className=" font-bold text-[64px] leading-[99%] tracking-[2%] uppercase text-white">2/5</span>
                </p>
                <p className="overflow-hidden ml-[-400px]">
                  <span className=" font-['Spline_Sans_Mono'] font-light text-[24px] leading-[1.3] tracking-[-2%] uppercase text-[#f7f7f7]">for me</span>
                </p>
                <p className="overflow-hidden">
                  <span className=" font-bold text-[36px] leading-[1.3] tracking-[-7%] uppercase text-[#f7f7f7]">dsgn/2</span>
                </p>
              </div>
              <div className="about-first-middle pb-10">
                <PhysicsText  />
              </div>
            </div>
            ...
            <div className="about-second__wrapper" >
              ...
              <div className="about-second__top ">
                <h4 className="overflow-hidden translate-y-[120px] px-10">
                ...
                <div className="about-second-text text-first w-max text-center mx-auto mb-[70px] translate-x-[-150px] pt-[150px]">
                  <div className="text-first__img mb-[25px] overflow-hidden w-[354px] h-[395px]">
                    <img
                      src={p1.src}
                      alt=""
                      className="w-full h-full scale-[1.3] clip-path-[polygon(0_0,100%_0,100%_100%,0_100%)] object-cover opacity-0"
                    />
                  </div>
                ...
                {/* Experience */}
                <div className="about-second-text text-second w-max text-center mx-auto translate-x-[-40px] pb-[125px]">
              ...
              {/* Philosophy */}
              <div className="about-second-title border-t border-[#aaa8a8] pt-[110px] relative z-[100] px-10">
              ...
              {/* Work description */}
              <div className="about-second-text text-first abs-t w-max px-10">
                <RevealLines
                  lines={[
              ...
              {/* Philosophy details */}
              <div className="about-second-text text-second ast-s w-max text-center ml-auto mr-[100px] mt-[60px]">
              ...
              {/* Lifestyle section */}
              <div className="about-second-text text-third w-max text-center mr-0 mt-[130px] px-10">
                <div className="text-third__wrapper flex">
                  <div className="text-third__img overflow-hidden w-[253px] h-[283px] mr-[64px]">
                    <img src={p1.src} alt="" className="w-full h-full object-cover scale-[1.5] clip-path-[polygon(0_0,100%_0,100%_100%,0_100%)]" />
                  </div>
                  <div className="text-third__img overflow-hidden w-[206px] h-[354px] mr-[235px]">
                    <img src={p1.src} alt="" className="w-full h-full object-cover scale-[1.5] clip-path-[polygon(0_0,100%_0,100%_100%,0_100%)]" />
                  </div>
              ...
              {/* Contact section */}
              <div className="about-second-text text-four flex mr-0 px-10">
                <a
                  href="#footer"
                  onClick={handleContactClick}
                  className="link-line about-link font-['Spline_Sans_Mono'] font-normal text-[26px] text-white mr-[190px] h-fit relative flex items-center"
                >
                  lets contact
              ...
              <div>
                  <div>
                    <p className='text-[80px] font-[Spline_Sans_Mono]font-light about-second-title'>HOOBIES</p>
                  </div>
                  <GoodreadsWidget />
              </div>

              {/* Spacer to ensure scrollable height */}
              <div className="spacer" style={{ height: '170vh' }} />
            </div>
          </div>
        </div>
      </section>
